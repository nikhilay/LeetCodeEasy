 ...
 * bad version are also bad.Suppose you have n versions [1, 2, ..., n] and you want to find out the first bad one, which
 * causes all the following ones to be bad.You are given an API bool isBadVersion(version) which will return whether
...
public class FirstBadVersion {
    public int firstBadVersion(int n) {
        int result = checkBad(1, n);
        return result;
    }

    private int checkBad(int low, int high) {
        if (low > high) return -1;
        int mid = low + (high - low) / 2;
        if (!isBadVersion(mid - 1) && isBadVersion(mid)) return mid;
        if (isBadVersion(mid)) {
            return checkBad(low, mid - 1);
        } else {
            return checkBad(mid + 1, high);
        }
